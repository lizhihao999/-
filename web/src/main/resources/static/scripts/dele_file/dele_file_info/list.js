layui.use(['form', 'layer', 'table', 'laytpl', 'laydate'], function () {
    ...
    var tableIns = table.render({
        elem: '#List',
        url: web.rootPath() + 'dele_file_info/list',
        cellMinWidth: 95,
        page: true,
        ...
        cols: [[
            {type: "checkbox", fixed: "left", width: 50},
                    ...
                    {field: 'str', title:  'str', minWidth: 100, align: "center"},
    ...
    table.on('toolbar(List-toolbar)', function (obj) {
        if (obj.event == 'add') {
            layer.open({
                ...
                fixed: false,
                maxmin: true,
                content: web.rootPath() + 'dele_file_info/add.html'
            });
        }
        ;
    });
        ...
        switch (obj.event) {
            case 'update':
                layer.open({
                    ...
                    fixed: false,
                    maxmin: true,
                    content: web.rootPath() + "dele_file_info/" + data + ".html?_"
                });
                break;
            case 'delete':
                var index = layer.confirm('确定要删除?', {
                    ...
                    $.ajax({
                        url: web.rootPath() + "dele_file_info/delete/" + data,
                        type: "delete",

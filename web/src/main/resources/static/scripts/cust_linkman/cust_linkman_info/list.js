layui.use(['form', 'layer', 'table', 'laytpl', 'laydate'], function () {
    ...
    var tableIns = table.render({
        elem: '#List',
        url: web.rootPath() + 'cust_linkman_info/list',
        cellMinWidth: 95,
        page: true,
    ...
    table.on('toolbar(List-toolbar)', function (obj) {
        if (obj.event == 'add') {
            layer.open({
                ...
                fixed: false,
                maxmin: true,
                content: web.rootPath() + 'cust_linkman_info/add.html'
            });
        }
        ;
    });
        ...
        switch (obj.event) {
            case 'update':
                layer.open({
                    ...
                    fixed: false,
                    maxmin: true,
                    content: web.rootPath() + "cust_linkman_info/" + data.id + ".html?_"
                });
                break;
            case 'delete':
                var index = layer.confirm('确定要删除?', {
                    ...
                    $.ajax({
                        url: web.rootPath() + "cust_linkman_info/delete/" + data.id,
                        type: "delete",
